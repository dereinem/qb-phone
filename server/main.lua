...
QBCore.Commands.Add('leitstelle', "Leitstelle besetzen (Polizei/EMS)", {}, false, function(source)
    ...
    if (PlayerData.job.name == 'police' or PlayerData.job.name == 'ambulance') and PlayerData.job.onduty then
        ...
        if ccData then
            ...
            if ccData.phoneNumber == PlayerData.charinfo.phone then
                ...
                TriggerClientEvent('QBCore:Notify', source, 'Du hast dich von der Leitstelle abgemeldet', 'error')
            else
                TriggerClientEvent('QBCore:Notify', source, 'Leitstelle ist besetzt von '..ccData.name, 'error')
            end
        else
            -- control center free
            ...
            TriggerClientEvent('QBCore:Notify', source, 'Du hast die Leitstelle erfolgreich übernommen', 'success')
        end
    end
end)

QBCore.Commands.Add('leitstelleclear', 'Log everyone out of Control Center (Admin/Boss only)', {{name = 'job', help = 'Job to clear Control Center'}}, true, function(source, args)
    local PlayerData = QBCore.Functions.GetPlayer(source).PlayerData
